...
- (void)viewDidLoad {
    ...
    [self.rootTableView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(64);
        make.size.equalTo(CGSizeMake(self.rootTableView.mj_w, self.rootTableView.mj_h));
    }];
}
...
- (void)backBarButtonItem {
    ...
    leftButton.imageEdgeInsets = UIEdgeInsetsMake(0, -35, 0, 0);
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:leftButton];
}

- (void)rightBarButtonItem{
    UIButton *rightButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 100, 30)];
    NSString *locationCity = [NSString stringWithFormat:@"当前定位:%@",[kCurrentCityInfoDefaults objectForKey:@"locationCity"]];
    [rightButton setTitle:locationCity forState:UIControlStateNormal];
    [rightButton setTintColor:[UIColor whiteColor]];
    rightButton.titleLabel.adaptiveFontSize = 17;
