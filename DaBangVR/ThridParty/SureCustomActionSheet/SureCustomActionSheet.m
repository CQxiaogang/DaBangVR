...
- (instancetype)initWithTitleView:(UIView *)titleView optionsArr:(NSArray *)optionsArr imgArr:(NSArray *)imgArr cancelTitle:(NSString *)cancelTitle selectedBlock:(void (^)(NSInteger))selectedBlock cancelBlock:(void (^)())cancelBlock {
    if (self = [super init]) {
        _headView      = titleView;
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        CGFloat imgViewW = cell.mj_h/2;
        UIImageView *imgView = [[UIImageView alloc] init];
        imgView.image = [UIImage imageNamed:_imgArr[indexPath.row]];
        [cell addSubview:imgView];
        [imgView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerY.equalTo(cell.mas_centerY);
            make.left.equalTo(kFit(120));
            make.height.equalTo(imgViewW);
            make.width.equalTo(imgViewW);
        }];
